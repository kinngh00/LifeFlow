...
현재 단계에는 기존 Prisma 모델과 마이그레이션을 바탕으로 데이터베이스 연결 상태 확인, 관리자용 지원제도 목록 조회 서비스, 지원제도와 최초 DRAFT 버전을 함께 생성하는 서비스, 실제 PostgreSQL 통합 테스트 환경이 포함됩니다.
...
- DB ID 중 ProgramVersion 식별자는 포함하지만 하위 generated ID, 생성·수정 시각과 관리자 ID는 제외합니다.
...
2026-07-19 기준, loopback 전용으로 격리한 PostgreSQL 18.3 테스트 DB에 기존 마이그레이션 2개를 적용하고 전체 통합 테스트 143건과 Playwright E2E 10건을 통과했습니다. Prisma Dev는 사용하지 않았습니다. Docker Desktop 엔진은 이미지 조회 시 HTTP 500을 반환해 이 검증에는 사용하지 않았으며, GitHub Actions는 아직 실제 실행하지 않았습니다.
...
GitHub Actions의 표준 PostgreSQL에서 전체 통합·관리자 E2E 테스트를 재검증한 뒤 비회원 설문과 추천 API·화면을 구현하는 것이 적절합니다. 실제 부산 지원제도 입력은 사용자 흐름이 확정된 이후로 유지합니다.
