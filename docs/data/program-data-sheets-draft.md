...
### 1차 개발 DB 입력·게시 검증 결과

- 실행일: 2026-07-19
- 대상: 위 5개 제도
- 저장 경로: 프로그램 생성 → DRAFT 구성 저장 → 규칙 테스트 → readiness → 게시 서비스
- 결과: 5개 모두 게시, 제도별 5/5 규칙 테스트 통과, readiness 통과
- 공개 검증: 추천 목록 5개, 상세 5개, 대표 공식 링크 5개 확인
- 신청 상태: 디딤돌카드+, 일하는 기쁨카드, 청년월세, 대학생 기숙사비는 `CLOSED`; 국민내일배움카드는 `ALWAYS_OPEN`
- 지역 검증: 중앙정부 2개는 `NATIONAL/null/null`, 부산시 2개는 `CITY_WIDE/26000/ALL`, 기숙사비는 신청자 거주지 `NATIONAL`과 부산 소재 대학 등 수동 검토를 분리
- 주의: 로컬 개발 PostgreSQL의 검증 결과이며 운영 DB 입력 또는 배포 결과가 아니다.

### 2차 개발 DB 입력·게시 검증 결과

- 실행일: 2026-07-19
- 대상: 정장대여서비스(드림옷장), 2026년 상반기 취업연수생, 국민취업지원제도 I유형 청년특례, 2026년 전세보증금반환보증 보증료 지원, 2026년 8월 머물자리론
- 결과: 5개 모두 게시, 제도별 5/5 규칙 테스트 통과, readiness 통과
- 누적 공개 데이터: 10개, DRAFT 0개, 공개 상세 10개 모두 대표 공식 출처 포함
- 신청 상태: 드림옷장·국민취업지원제도 `ALWAYS_OPEN`, 취업연수생 `CLOSED`, 보증료 지원 `NEEDS_CONFIRMATION`, 머물자리론 8월 회차 `UPCOMING`
- 보수적 판정: 취업연수생의 공개 본문에서 확정하지 못한 연령·학력·고용 세부조건과 보수는 생성하지 않고 `MANUAL_REVIEW` 및 `UNDETERMINED` 금액으로 저장
- 회차 분리: 머물자리론의 반복 월별 모집을 연속 기간으로 합치지 않고 2026년 8월 신청 회차로 저장
- 주의: 로컬 개발 PostgreSQL의 검증 결과이며 운영 DB 입력 또는 배포 결과가 아니다.

### 3차 검수 배치 확정 (2026-07-19)

| 제도 | 분야 | 범위 | 신청 유형 | 검수 판단 |
|---|---|---|---|---|
| 부산청년 취업성공풀(Full) 패키지 | 취업 | CITY_WIDE | 2026년 1~12월 | 월 단위 공식 기간을 1월 1일~12월 31일로 정규화하고 그 사실을 주의문에 명시 |
| LH 청년전세임대 | 주거 | NATIONAL | 2026년 1순위 수시모집 | 장기 제도와 2026년 회차성 공고를 분리 |
| 청년 주거급여 분리지급 | 주거 | NATIONAL | 상시 | 부모가구 수급·미혼·분리거주 사유는 수동 확인 |
| 청년전용 버팀목전세자금 | 주거 | NATIONAL | 상시 상품 | 계약일 기준 3개월 신청기한은 주의문과 수동 확인으로 분리 |
| 청년전용 보증부월세대출 | 주거 | NATIONAL | 상시 상품 | 계약일 기준 3개월 신청기한은 주의문과 수동 확인으로 분리 |

확정 근거:

- 취업성공풀 패키지는 부산청년플랫폼에서 39세 이하, 부산 주민등록, 지정 상담기관을 통한 취업, 6·9·12개월 근속지원과 최대 100만 원을 확인했다.
- LH 청년전세임대는 장기 제도 안내가 아니라 LH청약플러스의 `2026년 청년 전세임대 1순위 입주자 수시모집`을 버전으로 사용하며, 게시일 2026-02-24와 마감일 2026-12-31을 저장한다.
- 청년 주거급여 분리지급은 19세 이상 30세 미만, 주거급여 수급가구의 미혼자녀, 취학·구직 등에 따른 별도 거주와 임차 증빙을 공식 마이홈 안내에서 확인했다.
- 주택도시기금 두 상품은 통합검색 URL이 아닌 각각의 직접 상품 상세 URL을 대표 출처로 사용한다. 2026년 소득·자산 수치, 금리·한도는 상세 페이지에서 확인하되 자동 소득 계산은 하지 않는다.
- 소득·자산·신용, 세대주 정의, 중복대출, 1순위 유형과 실제 임대차·권리분석은 현재 설문과 단순 규칙으로 확정하지 않고 모두 `MANUAL_REVIEW`로 둔다.

### 3차 개발 DB 입력·게시 검증 결과

- 실행일: 2026-07-19
- 대상: 위 5개 제도
- 결과: 5개 모두 게시, 제도별 5/5 규칙 테스트 통과, readiness 통과
- 누적 공개 데이터: 15개, DRAFT 0개
- 공개 검증: 추천 결과 15개와 상태별 요약 15개 확인; LH 청년전세임대 상세에서 규칙 근거·신청 상태·공식 확인일·대표 출처 링크 확인
- 신청 상태: 취업성공풀 패키지·LH 청년전세임대 `OPEN`, 청년 주거급여 분리지급·주택도시기금 대출 2종 `ALWAYS_OPEN`
- 지역 검증: 부산 제도 1개는 `CITY_WIDE/26000/ALL`, 전국 제도 4개는 `NATIONAL/null/null`
- 주의: 로컬 개발 PostgreSQL과 로컬 브라우저의 검증 결과이며 운영 DB 입력 또는 배포 결과가 아니다.
...
- 신청·혜택 확정: 장기 제도와 `2026년 청년 전세임대 1순위 입주자 수시모집` 버전을 분리; 2026-02-24~2026-12-31, 현재 접수중
...
- 공식 출처: [주택도시기금 청년전용 버팀목전세자금](https://nhuf.molit.go.kr/FP/FP05/FP0502/FP05020301.jsp)
- 신청·혜택 확정: 최대 1억 5천만 원(임차보증금 80% 이내), 연 2.2~3.3%; 달력상 상시 상품이며 계약별 3개월 신청기한은 별도 확인
...
- 분야·범위: `YOUTH_HOUSING`, `NATIONAL/null/null`
- 주관·운영기관: 국토교통부 주택도시기금 / 기금 수탁은행
- 공식 출처: [주택도시기금 청년전용 보증부월세대출](https://nhuf.molit.go.kr/FP/FP05/FP0502/FP05020701.jsp)
- 신청·혜택 확정: 보증금 최대 4,500만 원, 월세 최대 1,200만 원; 달력상 상시 상품이며 계약별 3개월 신청기한은 별도 확인
- 자동 규칙 후보: AGE, INCOME_BAND, HOUSING, REGION(NATIONAL)
- MANUAL_REVIEW: 세대주·무주택, 소득·자산, 임대차계약, 중복대출, 신용·은행심사
